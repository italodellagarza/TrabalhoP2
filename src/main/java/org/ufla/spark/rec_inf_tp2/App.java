...
public class App {
	 ...
	 * VALUES -> { 10, 100, 1000 }
	 */
	private static int[] maxIters = new int[] { 10, 100, 1000 };
	/**
	...
	private static void testeCombincao() {
		int contModels = 0;
	...
	private static void testeModeloNaiveBayes() throws IOException {
		testeModelo(TipoBaseDeDados.TREINO_HDFS, TipoBaseDeDados.TESTE_HDFS, PreProcessamento.COMPLETO,
				ExtracaoFeatures.HASHING_TF_IDF, "teste", new ParamGridBuilder().build(), new NaiveBayes(),
	...
	private static void testeModelo(TipoBaseDeDados tipoBaseDeDadosTreino, TipoBaseDeDados tipoBaseDeDadosTeste,
		...
		CrossValidatorModel modeloCV = null;
		...
		OneVsRestModel bestModel = null;
		if (diretorioModelo.exists()) {
			modeloCV = CrossValidatorModel.load(nomeDoModelo);
			// bestModel = OneVsRestModel.load(nomeDoModelo + "/bestModel");
		...
		if (new File(predicoesNome).exists()) {
			...
			predicoes = modeloCV.transform(datasetTeste);
	...
	private static void testeModeloCombinandoParametros() throws IOException {
		...
		File diretorioPredicoesModFin = new File("predicoes_" + modeloGridSearch);
		File arquivoLog = new File("log_" + modeloGridSearch + ".txt");
		...
		for (int nFeatures : numFeatures) {
			...
			while (combinacaoParametros.temProximo()) {
				...
				System.out.printf("aggregationDepth: %d\n", parametros.aggregationDepth);
				System.out.printf("maxIter: %d\n", parametros.maxIter);
				System.out.printf("regParam: %f\n", parametros.regParam);
				System.out.printf("tol %f\n\n", parametros.tol);
				...
				OneVsRest ovr = new OneVsRest().setClassifier(lsvc);
				ParamMap[] parametrosGrid = new ParamGridBuilder()
						.addGrid(lsvc.aggregationDepth(), new int[] { parametros.aggregationDepth })
						.addGrid(lsvc.maxIter(), new int[] { parametros.maxIter })
						.addGrid(lsvc.regParam(), new double[] { parametros.regParam })
						.addGrid(lsvc.tol(), new double[] { parametros.tol }).build();
				CrossValidator cv = new CrossValidator().setEstimator(ovr).setEstimatorParamMaps(parametrosGrid)
				...
				if (melhorF1 < f1) {
					...
					bwLog.write(String.format("ATUALIZACAO F1!\n"));
					...
					bwLog.flush();
					modeloCV.write().save(diretorioModeloFinal.getAbsolutePath() + "_" + contModels);

					Dataset<Row> predicoes = modeloCV.bestModel().transform(datasetTesteFeatures);

					predicoes.write().save(diretorioPredicoesModFin.getAbsolutePath() + "_" + contModels);
					System.out.println("\nMODELO ATUALIZADO!\n");
				} else {
					bwLog.write(String.format("Modelo %d não melhora f1, f1 = %f\n", contModels, f1));
					bwLog.write(String.format("---------------------------------------------------\n"));
					bwLog.flush();
				}
				contModels++;
...
class CombinacaoParametros {
	...
	private int iAggregationDepth;
	private int iMaxIter;
	private int iRegParam;
	private int iTol;
